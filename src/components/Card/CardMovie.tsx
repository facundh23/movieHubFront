...
import { protectedRequest } from '../../api/request.service.ts';
  ...
  return (
    <div>
      <section className="w-65 h-70 mt-5 flex flex-col gap-2 items-center border-2 border-r-violet-400 border-y-black p-2">
        ...
        ⭐⭐⭐⭐
        {isAuthenticated ? <Link to="/home/:movieId" onClick={() => protectedRequest(getAccessTokenSilently)} className='bg-violet-400 p-2'>Go to details</Link> : <Link to="/" className='bg-violet-400 p-2'>Please Log in</Link>}
      </section>
    </div>
  )
}
